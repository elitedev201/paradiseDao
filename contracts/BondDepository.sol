...
contract UtopiaBondDepository is Ownable {
    ...
    IERC20 public immutable UTOP; // token given as payment for bond
    ...
    function deposit( 
        ...
        uint balanceBefore = UTOP.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            UTOP.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == UTOP.balanceOf(address(this)), "Not enough Time to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            UTOP.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                UTOP.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                UTOP.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return UTOP.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Utop supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = UTOP.totalSupply();
    ...
    /**
     *  @notice calculate amount of Utop available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Utop) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != UTOP, "NAT" );
